  ...
  return (
    <VerticalTimeline className="timeline-padding-force " layout="1-column">
      {dataCarrers.map((item, idx) => {
        ...
        const contentStyle =
          item.styleSet && item.styleSet.contentStyle
            ? item.styleSet.contentStyle
            : null;
        const contentArrowStyle =
          item.styleSet && item.styleSet.contentArrowStyle
            ? item.styleSet.contentArrowStyle
            : null;
        const iconStyle =
          item.styleSet && item.styleSet.iconStyle
            ? item.styleSet.iconStyle
            : defaultIconStyle;
        ...
        return (
          ...
          >
            <div className="mt-5 ml-5 text-sm text-left">
              <span className="px-5 py-1 text-white bg-black rounded-full">
                {item.period}
              </span>
            </div>
            <div
              className={`grid items-center justify-center grid-cols-1 ${clLgColsGrid}`}
            >
              {item.imgUrl && (
              ...
              <div className="m-5">
                {item.title && (
                  <h3 className="mb-2 text-2xl font-bold text-purple-500">
                    {item.title}
                  </h3>
                )}
                ...
                {item.where && (
                  <div className="flex items-center justify-between my-3">
                    <a
                      className="flex font-bold text-blue-700"
                      target="blank"
                      href={item.whereUrl}
                    >
